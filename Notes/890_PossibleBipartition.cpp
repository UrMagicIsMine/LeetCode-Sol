...
bool _DFS(vector<vector<int>>& graph, vector<int> &colors, int idx, int clr) {

	colors[idx] = clr;

	for (auto nxt : graph[idx]) {

		if (colors[nxt] == clr) return false;
		if (colors[nxt] == 0 && !_DFS(graph, colors, nxt, -clr))
			return false;
	}
	return true;
}
...
bool possibleBipartition(int N, vector<vector<int>>& dislikes) {
	...
	for (int i = 1; i <= N; i++) {
		if (colors[i] == 0 && _DFS(graph, colors, i, 1) == false) {
			return false;
		}
	}
	return true;
