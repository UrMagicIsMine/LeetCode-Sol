...
"fine" are similar, and "fine" and "good" are similar, "great" and "good" are
...
a sentence like words1 = ["great"] can never be similar to words2 = ["doubleplus","good"].
...
bool areSentencesSimilar(vector<string>& words1, vector<string>& words2,
  vector<pair<string, string>> pairs) {
    ...
    unordered_map<string, unordered_set<string>> hashtbl;
    for(auto& p : pairs){
        hashtbl[p.first].insert(p.second);
    }

    for(int i = 0; i < words1.size(); i++){
        if(words1[i] == words2[i])
            continue;
        auto it = hashtbl.find(words1[i]);
        if(it != hashtbl.end() && it->second.count(words2[i]))
            continue;
        it = hashtbl.find(words2[i]);
        if(it != hashtbl.end() && it->second.count(words1[i]))
            continue;
        return false;
    }
    return true;
