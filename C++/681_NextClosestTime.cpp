...
occurs 5 minutes later.  It is not 19:33, because this occurs 23 hours and 59
minutes later.
Example 2:
...
may be assumed that the returned time is next day's time since it is smaller than
...
string nextClosestTime(string time) {
    vector<int> mod = {600, 60, 10, 1};

    int pos = time.find(':');
    int cur = stoi(time.substr(0, pos)) * 60 + stoi(time.substr(pos+1));
    string next = "0000";
    int d = 0;
