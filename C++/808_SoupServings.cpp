...
* Output: 0.625
* Explanation:
* If we choose the first two operations, A will become empty first. For the third
* operation, A and B will become empty at the same time. For the fourth operation,
* B will become empty first. So the total probability of A becoming empty first
* plus half the probability that A and B become empty at the same time, is
...
double _DFS(vector<vector<double>> & dp, int A, int B) {
	...
	dp[A][B] = 0.25 * ( _DFS(dp, A - 4, B) + _DFS(dp, A - 3, B - 1)
                     + _DFS(dp, A - 2, B - 2) + _DFS(dp, A - 1, B - 3) );
	return dp[A][B];
}
...
double soupServings(int N) {
	...
	const int MAX = 241;
	vector<vector<double>> dp(MAX, vector<double>(MAX, 0.0));
...
int main()
{
	double ret = soupServings(600);
	double ans = 0.93593;
	assert(abs(ret == ans) < 1e-5);
	return 0;
}
