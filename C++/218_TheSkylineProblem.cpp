...
0 ≤ Li, Ri ≤ INT_MAX, 0 < Hi ≤ INT_MAX, and Ri - Li > 0. You may assume all
...
vector<pair<int, int>> getSkyline(vector<vector<int>>& buildings) {
    ...
    for(auto& bldg : buildings){
        events.push_back({bldg[0],  bldg[2]});
        events.push_back({bldg[1], -bldg[2]});
    }

    auto lambda = [](const pii_t & p1, const pii_t & p2){
    ...
    for(auto & p : events){
        ...
        int x = p.first;
        int h = abs(p.second);
        bool entering = (p.second > 0);

        if(entering){
            // entering new building;
            if(h > _maxHeight(heights)){
                resl.push_back(p);
            }
            heights.insert(h);
        }
            ...
            // leaving
            auto range = heights.equal_range(h);
            heights.erase(range.first);
            int mh = _maxHeight(heights);
            if(h > mh){
                resl.emplace_back(x, mh);
            }
        }
    }
    return resl;
...
int _maxHeight(multiset<int>& heights){
    if(heights.empty())
        return 0;
    else
